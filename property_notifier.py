...
class ResidentialListingsSearch:
    def __init__(self,user,api_client = APIClient()):
        self.user = user
        self.api_client = api_client
        self.responses = [] # Stores json responses for each location
    
    def listings_request(self,user):
        for location in user.locations:
            for suburb,state,postcode in location.items():
                listings = requests.post('https://api.domain.com.au/v1/listings/residential/_search',json = {
                    ...
                    'pageSize': 50,
                    'listedSince': f'{user.date_posted}'}, headers = self.api_client.access_token)
                
                self.responses.append(listings.json())

# Stores and retrieves property listing data from Residential_Listings_Search
class PropertyData:
    def __init__(self,user,residential_listings_search):
        self.user = user
    ...
    def common_listings_data(self, residential_listings_search):
            ...
            for properties in listings_json:
                if properties['type'] == 'PropertyListing':
                    ...
                    if display_price_search == None:
                        display_price = self.user.min_price
                    else:
    ...
    # Weekly and annual rent data for renters
    ...
    # Convert property listings with prices that contain symbols into integers
...
class SuburbPerformanceStatistics:
    def __init__(self, user, property_data, api_client = APIClient()):
        self.user = user
    ...
    def house_statistics(self,index):
        self.request_json(index,self.user,'house')
...
class MetricsCalculation:
    def __init__(self, user, property_data, suburb_performance_statistics):
        self.user = user
        self.data = property_data.data
    ...
    def calculate_investor_metrics(self,index,property_category):
        ...
        if self.user.variable_loan_type =='Interest Only':
            self.user[index]['Mortgage Repayments'] = round(self.data[index]['Price'] * (self.user.lvr / 100) * monthly_decimal_rate,2)
        else:
...
class Customer_Email:
    def __init__(self,user,property_data, email_address=None,password=None): # Email + Password from session data in flask application
        self.user = user
        self.data = property_data.data
