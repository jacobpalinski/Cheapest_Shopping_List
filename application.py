...
# Form for /investor page, uses same fields as /renter page + additional fields
...
# Form for /owneroccupier page, uses same fields as /investor page
class OwnerOccupierForm(InvestorForm):
    pass

# Submit field for /renter, /investor and /owneroccupier pages
...
def initial_page():
    ...
    return render_template('initial.html',form = initial_form)
...
def renter():
    ...
    if request.method == 'POST':
        if renter_form.validate_on_submit():
            ...
            # Returns flash error message and reloads page when no data exists for given form criterea
...
def investor():
    ...
    if request.method == 'POST':
        if investor_form.validate_on_submit():
            ...
            # Generate key investment metrics for all properties that match criterea
            for index in range(len(suburb_performance_statistics.data)):
                if suburb_performance_statistics.data[index]['Property Type'] != 'ApartmentUnitFlat': # Will be either House or Townhouse 
...
def owner_occupier():
    ...
    if request.method == 'POST':
        if owner_occupier_form.validate_on_submit():
            ...
            # Generate key owner occupier metrics for all properties that match criterea
            for index in range(len(suburb_performance_statistics.data)):
                if suburb_performance_statistics.data[index]['Property Type'] != 'ApartmentUnitFlat': # Will be either House or Townhouse 
                    if not suburb_performance_statistics.api_calls_made.get(('House', suburb_performance_statistics.data[index]['Suburb'], suburb_performance_statistics.data[index]['State'], suburb_performance_statistics.data[index]['Postcode'])):
                        suburb_performance_statistics.house_statistics(index)
